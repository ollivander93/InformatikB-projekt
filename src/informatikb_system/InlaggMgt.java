...
public class InlaggMgt {
    ...
    public void showSocInlagg1(StyledDocument d, String amne) {
        ...
        if (!(list == null)) {
            for (int i = list.size() - 1; i >= 0; i--) {
                ...
                if (vis.equals("true") && !sokvag.equals("Ingen fil")) {
                    try {
                        d.insertString(0, "\n" + tid + "  " + datum + "\n", AtrSet2);
                        d.insertString(0, "Ignore", style);
                ...
                } else if (sokvag.equals("Ingen fil")) {
                    try {
    ...
    public ArrayList<HashMap<String, String>> hamtaInlagg(String amne) {
        ...
        String sqlFraga = "select ANSTALLD.FIRST_NAME, ANSTALLD.LAST_NAME, INLAGG.DATUM, INLAGG.TID, INLAGG.TEXT, INLAGG.TITEL, INLAGG.VISIBLE, INLAGG.FILSOKVAG "
                + "FROM ANSTALLD "
                + "JOIN INLAGG "
    ...
    public String getSokVag(String iid) {
        String sqlFraga = "select inlagg.sokvag from inlagg where inlagg.iid = " + iid + ";";
        String sokvag = "";
        try {
            sokvag = idb.fetchSingle(sqlFraga);
        } catch (InfException e) {
            System.out.println(e.getMessage());
        }
        return sokvag;
